...
% Modified by : Pavan Poojar -Included golden angle
% Modified by: Sairam Geethanath, MIRC & CMRRP - integrated with Pulseq -
% still flaky - heavily dependent on choice of alpha
 
function [kshot,Gn,lambda]= vds2D_pulseq_v1_N_GA_TGA(FOV,N,Nshots,alpha,lims,phiN)
  ...
  if(mod(length(kn),2)~=0)
      ...
      Gn = zeros(length(kn), Nshots);
            ...
            for s=1:Nshots
                ...
                if phiN==0
                    kshot(:,s) =kn.*exp(2*pi*1i*s/Nshots);%m-1 % Normal angle
                else
                    kshot(:,s) =kn.*exp(1i*mod(phiN*s,2*pi));% GA/TGA            
                end
...
%                 Gx =[0 Gx];Gy =[0 Gy];
                ...
                plot(squeeze(kshot(:,s)),'Linewidth',1.5);hold on;grid on;xlabel('kx (m-1)'); ylabel('ky(m-1)');pause(0.5);
                disp(s);
            end
