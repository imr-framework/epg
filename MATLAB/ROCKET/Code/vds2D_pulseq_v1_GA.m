...
figure(1030);
  if(mod(length(kn),2)~=0)
      ...
      Gn = zeros(length(kn), Nshots);
            ...
            for s=1:Nshots
                %kshot(:,s) =kn.*exp(2*pi*1i2*pi*1i*s/Nshots*s/Nshots);%m-1
                kshot(:,s) =kn.*exp(2i*pi*mod(phiN*(180/(pi))*s,360));% Golden angle
...
%                 Gx =[0 Gx];Gy =[0 Gy];
                ...
                plot(squeeze(kshot(:,s)));hold on;grid on;xlabel('kx (m-1)'); ylabel('ky(m-1)');axis([-1000 1000 -1000 1000]);
                disp(s);
            end
