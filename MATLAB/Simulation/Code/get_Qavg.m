function [Qavg_df] = get_Qavg(Mass_cell,Mdef,Q,ind)
...
% if((Mass_cell(x,y,z) > Mass_air)) ind already represents cell > Mass_air
...
%                 [Qt]= getQs(Q,Mass_cell,dim,x,y,z,f,Mdef);
                ...
                Qavg(k1,:,:) = ((squeeze(Qcorner).*(f^3)) + (squeeze(Qedge).*(f^2)) + (squeeze(Qface).*f) + (squeeze(Qcore)))./Mdef;
                       if(mod(k1,1e3)==0) %Only for debug
                           disp(k1*100/length(ind));
                       end
             ...
             Qavg_df = zeros(M*N*P,8,8);
             Qavg_df(ind,:,:) = squeeze(Qavg(1:end,:,:));
